...
{
    slapi_log_error(SLAPI_LOG_ERR, "testpreop_add",
                  "*** PREOPERATION ADD PLUGIN ***\n");
    Slapi_Entry *e;
    Slapi_Attr *a;
    Slapi_Value *v;
    struct berval **bvals;
    ...
    int len;
    const char* str = slapi_entry2str(e, &len);

    int n = slapi_entry_first_attr(e, &a);

    if (n != 0)
    {
        slapi_log_error(SLAPI_LOG_PLUGIN,
                      "testpreop_add", "Could not get first entry\n");
        return 0;
    }
    
    Slapi_Attr *b = NULL;
    const char* strValue1;
    const char* strValue2;
    char* type;

    while (1)
    {
        slapi_attr_get_type(a, &type);        

        Slapi_Value *value;
        int hint = slapi_attr_first_value(a, &v);
        strValue1 = slapi_value_get_string(v);
        ...
        while (1)
        {
            ...
            strValue2 = slapi_value_get_string(v);        
        }

        if (slapi_entry_next_attr(e, a, &b) == 0)
        {
            a = b;
            b = NULL;
        }
